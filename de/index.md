{% include google-analytics.html %}
{% include change-language.md %}
## Über mich

Hallo! Ich bin Adrian und habe **RowerTrain** entwickelt – eine App für verbundene Ruderer, die dazu konzipiert ist, dein Trainingserlebnis zu verbessern und deine Ruderleistung zu steigern.

Ich habe diese App ursprünglich für Decathlon Domyos Ruderer geschrieben. Es stellte sich heraus, dass Decathlon beschlossen hat, die Domyos eConnected-App nicht mehr zu warten und Benutzer zur Kinomap-App zu leiten. Ich fand diese App nicht sehr nützlich, also beschloss ich, meine eigene zu schaffen.

RowerTrain funktioniert mit einer breiten Palette von Ruderergometern, einschließlich beliebter Modelle wie Domyos 500, 900, Woodrower sowie anderer Marken wie Concept2, Christopeit, Merach und weitere. Wenn dein Ruderer nicht mit RowerTrain funktioniert, teile mir das bitte mit. Manchmal entsprechen Ruderer, besonders Einstiegsmodelle, nicht vollständig dem FTMS-Protokoll. Aber dank des Benutzer-Feedbacks konnte ich einige Probleme beheben und die Kompatibilität verbessern.

Verbinde einfach deinen Ruderergometer über Bluetooth mit deinem Smartphone und starten dein Rudertraining. Dein Ruderer muss das Bluetooth FTMS (Fitness Machine Service) Protokoll unterstützen, um mit RowerTrain zu funktionieren.

## Hinweis zu den vorgeschlagenen Trainingssitzungen

Die angebotenen Trainingssitzungen dienen als Beispiele. Falls du diese nicht passend findest, kannst du deine eigenen Trainingssitzungen mit dem integrierten Editor erstellen. Du kannst mir auch bekannte Probleme und Fehler mitteilen, damit ich diese beheben kann.

## Kontakt & Feedback

Ich bin offen für Vorschläge und Verbesserungen! Zögere nicht, mich zu kontaktieren:

**E-Mail:** powertrainapp.sup [at] gmail.com

**Github-Probleme:** Öffne ein [Issue](https://github.com/iliuta/iliuta.github.io/issues) auf GitHub

## RowerTrain App herunterladen

[![Auf Google Play erhalten](https://img.shields.io/badge/Get%20it%20on-Google%20Play-blue?style=for-the-badge&logo=google-play)](https://play.google.com/store/apps/details?id=com.iliuta.ftms)

Ich verbringe viel Zeit damit, diese App zu warten und sie so nützlich wie möglich zu machen. Sie ist kostenlos auf Google Play verfügbar. Wenn Sie die Entwicklung der App unterstützen möchten, können Sie eine Spende über [Buy Me a Coffee](https://www.buymeacoffee.com/iliuta) machen. Es ist nicht obligatorisch, aber die Wertschätzungen machen mir immer Freude und motivieren mich, weiterzumachen. Zögern Sie nicht, einen Kommentar auf Google Play zu hinterlassen, um die App noch sichtbarer und beliebter zu machen.

[![Im App Store herunterladen](https://img.shields.io/badge/Download_on_the-App_Store-black?style=for-the-badge&logo=apple)](https://apps.apple.com/app/rowertrain/id6758657773)

Die App ist auch im Apple App Store erhältlich. Sie können sie kostenlos herunterladen und den Modus "Freies Fahren" nutzen. Dies ermöglicht es Ihnen, die App zu testen und zu sehen, ob sie mit Ihrem Ruderer funktioniert. Um andere Funktionen zu nutzen, müssen Sie jedoch einen In-App-Kauf durchführen.

## Benutzerhandbuch

[Dokumentation](https://iliuta.github.io/rowertrain-docs/index_de.html)


## Funktionen

- **Freies Fahren**: Definiere Rude- oder Zeitdauer-Ziele, wähle virtuelle Strecken, definiere Tempolimit und/oder Schlagfrequenzziele und starten dein Rudertraining
- **Trainingssitzungs-Generator**: Generiere strukturierte Rudertraining-Sitzungen aus vordefinierten Trainingsvorlagen
- **Gespeicherte Sitzungen folgen**: Wähle aus integrierten Rudertrainings oder benutzerdefinierten Rudertraining-Sitzungen, die du erstellt hast
- **Rudertraining-Editor mit Intervalltraining**: Erstelle und personalisiere Ruder-Intervalltrainingssitzungen. Definiere Schlagfrequenz- und Tempolimit-Ziele für jedes Ruderintervall
- **Pulsuhr-Integration**: Verbinde Bluetooth-Pulsmesser, um deine Herzfrequenz während des Rudertrainings zu überwachen
- **Strava-Integration**: Synchronisiere automatisch deine Rudertrainings-Daten mit Strava
- **Virtuelle Ruderrouten**: Wähle aus einer Vielzahl vordefinierter Ruderrouten und visualisiere sie auf Strava, als würdest du im Freien rudern.

## Bildschirmfotos

{% include screenshots.md %}

---

## Schlüsselwörter & Tags

**Ruder-App**, **Indoor-Rudern**, **Rudertraining**, **Fitness-App**, **Rudermaschinen-App**, **Ruderergometer**, **Rudersitzung**, **Rudersoftware**, **Rudertrainer**, **Rudertimer**, **Ruder-GPS**, **Ruder-Bluetooth**, **FTMS-Rudern**, **Ruder-Intervalltraining**, **Ruder-Herzfrequenz**, **Ruder-Strava**, **Ruderleistung**, **Ruder-Analytik**, **Domyos-Rudern**, **Concept2-Rudern**, **Ruderausrüstung**, **Ruder-Fitness**, **Ruderübung**, **Ruder-Cardio**, **Indoor-Rudermaschine**, **Rudertrainings-App**, **Rudertraining-Generator**, **Ruder-Tempotraining**, **Ruder-Schlagfrequenz**, **Ruder-Leistungsmesser**
